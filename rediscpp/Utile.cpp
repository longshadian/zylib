...
RedisContextGuard redisConnect(std::string ip, int port)
{
    return RedisContextGuard(::redisConnect(ip.c_str(), port));
}

RedisContextGuard redisConnectWithTimeout(std::string ip, int port, int seconds, int microseconds)
{
    struct timeval tv;
    tv.tv_sec = seconds;
    tv.tv_usec = microseconds;
    return RedisContextGuard(::redisConnectWithTimeout(ip.c_str(), port, tv));
}

int redisGetReply(redisContext* context, RedisReplyGuard* guard)
{
    redisReply* reply = nullptr;
    int ret = ::redisGetReply(context, (void**)&reply);
    if (ret != REDIS_OK)
        return ret;
    *guard = RedisReplyGuard(reply);
    return ret;
}
...
long long DEL(RedisContextGuard& context, std::string key)
{
...
long long DEL(RedisContextGuard& context, std::vector<std::string> keys)
{
...
long long DEL(RedisContextGuard& context, Buffer key)
{
    std::vector<Buffer> temp;
    temp.emplace_back(std::move(key));
...
long long DEL(RedisContextGuard& context, std::vector<Buffer> keys)
{
    if (keys.empty()) {
        return 0;
    }
