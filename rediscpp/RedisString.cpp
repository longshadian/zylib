...
RedisString::RedisString(RedisContextGuard& context)
    : m_context(context)
{}
...
std::vector<Buffer> RedisString::KEYS(Buffer key)
{
    RedisReplyGuard reply{ reinterpret_cast<redisReply*>(
        ::redisCommand(m_context.get(),"KEYS %b", key.getData(), key.getLen())
        )
    };
    if (!reply)
        throw RedisReplyExceiption("KEYS reply null");
    if (reply->type == REDIS_REPLY_ERROR)
        throw RedisReplyExceiption(reply->str);
    if (reply->type != REDIS_REPLY_ARRAY)
        throw RedisReplyExceiption("KEYS type REDIS_REPLY_ARRAY");
