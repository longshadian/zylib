...
bool NetworkService::init()
{
    ...
    LOG(INFO) << "start network service max_connection:" << server_conf->m_max_count;
...
void NetworkService::sendMessage(network::ConnectionHdl hdl,
    int32_t msg_id, const std::string& msg)
{
    if (msg.empty())
        return;
    ...
    int32_t total_len = 4 + 4 + msg.size();
    std::vector<uint8_t> buffer{};
    buffer.resize(total_len);
    std::memcpy(buffer.data(), &total_len, 4);
    std::memcpy(buffer.data() + 4, &msg_id, 4);
    std::memcpy(buffer.data() + 8, msg.data(), msg.size());
