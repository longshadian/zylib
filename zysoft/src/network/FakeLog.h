...
inline int Snprintf(char* buf, std::size_t buflen, const char* format, ...)
{
    int r;
    va_list ap;
    va_start(ap, format);
...
inline int Vsnprintf(char* buf, std::size_t buflen, const char* format, va_list ap)
{
    int r;
    if (!buflen)
        return 0;
...
inline void FlushLog(int severity, const char *msg)
{
    if (g_log_callback) {
...
inline void PrintfLogv(int severity, const char* errstr, const char* fmt, va_list ap)
{
    char buf[1024];
    std::size_t len;
...
inline const char* SeverityString(int severity)
{
...
inline void PrintfLog(int severity, const char* fmt, ...)
#ifdef __GNUC__
...
#endif
{
    va_list ap;
    va_start(ap, fmt);
    detail::PrintfLogv(severity, nullptr, fmt, ap);
    va_end(ap);
}
