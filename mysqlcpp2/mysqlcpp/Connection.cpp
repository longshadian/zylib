...
MYSQL* Connection::RawMYSQL() { return m_mysql; }
...
PreparedStatementPtr Connection::CreatePreparedStatement(const char* sql)
{
    return ParsePreparedStatement(sql, std::strlen(sql));
}

PreparedStatementPtr Connection::CreatePreparedStatement(const std::string& sql)
{
    return ParsePreparedStatement(sql.c_str(), sql.size());
}

PreparedStatementPtr Connection::CreatePreparedStatement(std::string_view sql)
{
    return ParsePreparedStatement(sql.data(), sql.size());
}

PreparedStatementPtr Connection::ParsePreparedStatement(const char* sql, size_t sql_len)
{
    MYSQL_STMT* stmt = ::mysql_stmt_init(RawMYSQL());
    if (!stmt) {
        uint32 err_no = ::mysql_errno(RawMYSQL());
        const char* err_str = ::mysql_error(RawMYSQL());
