...
private:
    ...
    {
        for(std::size_t i = 0, n = lhs.length(); i < n; ++i)
        {
            if(lhs[i] != rhs[i])
            {
                return false;
    ...
    static std::size_t get_length_(string_type const& s)
    {
        return s.length();
    }
    ...
    /// Returns the length of the delimiter
    static std::size_t get_length_(char const /* delim */)
    {
        return 1;
    }
    /// Returns the length of the delimiter
    static std::size_t get_length_(wchar_t const /* delim */)
    {
        return 1;
    }
...
public:
    ...
    /// Returns the length of the delimiter
    static std::size_t length(delimiter_type const& delim)
    {
...
public:
    ...
    typedef S                                               string_type;                /// The sequence string type
    typedef D                                               delimiter_type;             /// The delimiter type
    typedef B                                               blanks_policy_type;         /// The blanks policy type
#ifndef STLSOFT_DOCUMENTATION_SKIP_SECTION
    typedef B                                               ignore_blanks_type;
#endif /* !STLSOFT_DOCUMENTATION_SKIP_SECTION */
    typedef V                                               value_type;                 /// The value type
    typedef T                                               traits_type;                /// The traits type
    typedef P                                               comparator_type;            /// The tokeniser comparator type
    typedef typename traits_type::value_type                char_type;                  /// The character type
    /// The size type
    ///
...
public:
    ...
    string_tokeniser(const char_type* psz, const delimiter_type& delim)
        : m_str(psz)
        , m_delimiter(delim)
    {
    }
...
// they are correctly discriminated
    ...
    string_tokeniser(const string_type& str, const delimiter_type& delim)
        : m_str(str)
        , m_delimiter(delim)
    {
    }
...
// Define the template overload if member template ctors are supported
    ...
    string_tokeniser(const S1& str, const delimiter_type& delim)
    ...
    /// \param delim The delimiter to use
    template <typename I>
    string_tokeniser(I from, I to, const delimiter_type& delim)
        : m_str(from, to)
        , m_delimiter(delim)
    {
    }
    ...
    private:
        ...
        void skip_blanks_if_(skip_discriminator_type_<1>)
        {
            ...
            for(m_find0 = m_next; m_find0 != m_end; )
            {
                ...
                {
                    m_find0 +=  static_cast<std::ptrdiff_t>(m_cchDelimiter);
                }
            }
        }
        ...
        void determine_end_()
        {
            ...
            for(m_find1 = m_find0; ; )
            {
                ...
                else
                {
                    ...
                    m_next = m_find1 + static_cast<std::ptrdiff_t>(m_cchDelimiter);
