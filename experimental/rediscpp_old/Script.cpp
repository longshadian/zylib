...
Script::Script(RedisContextGuard& context)
    : m_context(context)
{}
...
Buffer Script::EVAL(Buffer cmd, std::vector<Buffer> keys, std::vector<Buffer> values)
{
    std::string evalsha = "EVAL";
    return evalInternal(std::move(evalsha), std::move(cmd), std::move(keys), std::move(values));
}

Buffer Script::EVALSHA(Buffer cmd, std::vector<Buffer> keys,
...
Buffer Script::evalInternal(std::string eval_cmd, Buffer cmd,
...
Buffer Script::luaToRedis(const redisReply* reply)
{
    /*
    ...
    */
    ...
    case REDIS_REPLY_STRING: {
        return replyToRedisBuffer(reply);
    }
    case REDIS_REPLY_ARRAY: {
        Buffer ret(Buffer::ARRAY);
        for (size_t i = 0; i != reply->elements; ++i) {
            ret.pushBack(luaToRedis(reply->element[i]));
        }
        return ret;
    }
    case REDIS_REPLY_INTEGER: {
        return Buffer(std::to_string(reply->integer));
    }
