...
bool TcpClient::Start()
{
    ...
    auto socket = std::make_shared<boost::asio::ip::tcp::socket>(m_io_pool.NextIOContext()->m_io_context);
...
bool TcpClient::Reconnect()
{
    ...
    auto socket = std::make_shared<boost::asio::ip::tcp::socket>(ioc->m_io_context);
    socket->async_connect(m_server_addr, [this, socket](const boost::system::error_code& ec) 
        {
            if (ec) {
                m_is_connected.store(false);
