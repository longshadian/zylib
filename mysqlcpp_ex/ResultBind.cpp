...
bool ResultBind::bindResult(PreparedStatement& ps)
{
    ...
    m_is_null.resize(num_fields);
    std::memset(m_is_null.data(), 0, sizeof(my_bool) * num_fields);

    m_err.resize(num_fields);
    std::memset(m_err.data(), 0, sizeof(my_bool) * num_fields);

    m_len.resize(num_fields);
    ...
    for (size_t i = 0; i < num_fields; ++i) {
        ...
        m_bind[i].error		= &m_err[i];
        m_bind[i].is_unsigned= field->flags & UNSIGNED_FLAG;
    }

    if (::mysql_stmt_bind_result(ps.getMYSQL_STMT(), m_bind.data())) {
        FAKE_LOG(ERROR) << "mysql_stmt_bind_result, cannot bind result from MySQL server. Error: " 
